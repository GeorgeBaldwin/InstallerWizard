...
#Region- WinAPI Functions. Comment entire this out if you have already included WinAPI.au3
; Kernel32 is always exists so @error will always = 0. Check for this is totally useless
;Func _WinAPI_GetProcAddress($hModule, $vName)
;	Local $sType = "STR"
;	If IsNumber($vName) Then $sType = "WORD" ; if ordinal value passed
;	Return DllCall("kernel32.dll", "PTR", "GetProcAddress", "HANDLE", $hModule, $sType, $vName)[0]
;EndFunc

;Func _WinAPI_LoadLibrary($sFileName)
;	Return DllCall("kernel32.dll", "HANDLE", "LoadLibraryW", "WSTR", $sFileName)[0]
;EndFunc
;
;Func _WinAPI_FreeLibrary($hModule)
;	Return DllCall("kernel32.dll", "BOOL", "FreeLibrary", "HANDLE", $hModule)[0]
;EndFunc

; user32.dll always exists and is always loaded by a thread using msg-loop. Check @error is suck!
;Func _WinAPI_GetDlgCtrlID($hWnd)
;	Return DllCall("user32.dll", "INT", "GetDlgCtrlID", "HWND", $hWnd)[0]
;EndFunc
;
;Func _WinAPI_SetCapture($hWnd)
;	Return DllCall("user32.dll", "HWND", "SetCapture", "HWND", $hWnd)[0]
;EndFunc
;
;Func _WinAPI_DestroyWindow($hWnd)
;	Return DllCall("user32.dll", "BOOL", "DestroyWindow", "HWND", $hWnd)[0]
;EndFunc
#EndRegion

#Region WinAPIShellEx Functions. Comment this region out if you have already included WinAPIShellEx.au3
; comctl32.dll is checked when this file is loaded, we assume that it exists and DllCall should never fail!
Func _WinAPI_SetWindowSubclass($hWnd, $pSubclassProc, $idSubClass, $pData = 0)
	Return DllCall('comctl32.dll', 'BOOL', 'SetWindowSubclass', _
				'HWND', $hWnd, 'PTR', $pSubclassProc, 'UINT_PTR', $idSubClass, 'DWORD_PTR', $pData)[0]
EndFunc

Func _WinAPI_DefSubclassProc($hWnd, $iMsg, $wParam, $lParam)
	Return DllCall('comctl32.dll', 'LRESULT', 'DefSubclassProc', _
				'HWND', $hWnd, 'UINT', $iMsg, 'WPARAM', $wParam, 'LPARAM', $lParam)[0]
EndFunc   ;==>_WinAPI_DefSubclassProc

Func _WinAPI_RemoveWindowSubclass($hWnd, $pSubclassProc, $idSubClass)
	Return DllCall('comctl32.dll', 'BOOL', 'RemoveWindowSubclass', _
				'HWND', $hWnd, 'PTR', $pSubclassProc, 'UINT_PTR', $idSubClass)
EndFunc
#EndRegion
